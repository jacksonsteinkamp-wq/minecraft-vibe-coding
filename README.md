...
```
You'll be prompted for a Discord bot token, a channel ID, and a hotkey. Config is saved to `clip_recorder_config.json` so it's one-time.
...
**Notes:**
- `clips/` and `clip_recorder_config.json` are in `.gitignore` — videos are large binaries and the config contains your bot token, so neither should ever be pushed to git
